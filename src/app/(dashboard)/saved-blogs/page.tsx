...
import { BlogsDataTableType } from "../../types/blogType";
import { Pagination } from "../../types/userType";
import BlogDialog from "@/src/components/dialog/blog";
import { useSession } from "next-auth/react";
import { GetMyBlogs } from "@/src/services/blog.service";
import { SpinnerCustom } from "@/src/components/loading";
import { GetMyBookmarks } from "@/src/services/bookmark.service";
...
const SavedBlogPage = () => {
  ...
  const fetchSavedBlogs = async (search = searchQuery, currentPage = page) => {
    try {
      setLoadingMain(true);
  ...
  const handleDialogSuccess = async () => {
    ...
    await fetchSavedBlogs(searchQuery, page);
  };
    ...
    <div className="w-full">
      <div className="mb-6 flex w-full flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center lg:w-auto">
          <p className="text-xl font-semibold">Saved Blogs</p>
          ...
          <div className="grid w-full grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {dataTable.map((blog) => (
              <BlogCard
                ...
                onUpdated={() => {
                  fetchSavedBlogs(searchQuery, page);
                }}
                onDeleted={(id) => {
